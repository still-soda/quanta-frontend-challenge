...
import { HoverTriggerFlowData } from '@challenge/core';
import { ExtractData } from '../../utils/extract-data.util';
import TRIGGER_FLOW_CONFIG from './config';

defineProps<NodeProps<HoverTriggerFlowData>>();

const headerConfig = TRIGGER_FLOW_CONFIG['trigger:wait'];

const node = useNode<HoverTriggerFlowData>().node;
const data = reactive<ExtractData<HoverTriggerFlowData>>({
   selector: '',
});
