...
import { FocusTriggerFlowData } from '@challenge/core';
import { ExtractData } from '../../utils/extract-data.util';
import TRIGGER_FLOW_CONFIG from './config';

defineProps<NodeProps<FocusTriggerFlowData>>();

const headerConfig = TRIGGER_FLOW_CONFIG['trigger:wait'];

const node = useNode<FocusTriggerFlowData>().node;
const data = reactive<ExtractData<FocusTriggerFlowData>>({
   selector: '',
});
